  ...
  return (
    <div>
      ...
      <div className="grid">
        <ModuleBox>
          <h3>Оплачено</h3>
          <p>{order.list.length} позиции на сумму: {order.price} {set_currency}</p>
          <p>Доставка: {order.deliveryPrice} {set_currency}</p>
          <p>Итого: {order.total} {set_currency}</p>
          <p>Тип оплаты: {order.payment === 'acquiring' ? 'Эквайринг' : 'Счет для юридических лиц'}</p>
        </ModuleBox>
        <ModuleBox>
          <h3>Получатель</h3>
          <p>ФИО: {order.name}</p>
        ...
        <ModuleBox>
          ...
          {order.addition && <p>Дополнительно:<br /> {order.addition}</p>}
        </ModuleBox>
      </div>
